...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.norm_layer = nn.InstanceNorm1d(c_h, affine=False)
        self.spattention_layers = nn.ModuleList([SPAttention(c_h) for _ in range(n_conv_blocks * 2)])
        self.dropout_layer = nn.Dropout(p=dropout_rate)
        self.post_net = nn.Sequential(
            nn.Conv1d(80, 512, kernel_size=5, padding=2),
            # nn.InstanceNorm1d(512, affine=False),
            nn.Tanh(),
            nn.Dropout(0.5),
            ...
            # nn.InstanceNorm1d(80, affine=False),
            nn.Dropout(0.5),
        )
    ...
    def unet_block(self, inp, cond, z):
        ...
        for l in range(self.n_conv_blocks):
            ...
            if self.upsample[l] == 1:
                y = y + self.norm_layer(pad_layer((z[- ((l + 1) // 2 + 1)]), self.z_affine_layers[l // 2]))

            y = pad_layer(y, self.first_conv_layers[l])
            y = self.norm_layer(y)
            y = self.act(y)
            y = self.dropout_layer(y)
            y = y + pad_layer(self.spattention_layers[2 * l](y, cond), self.cond_affine_layers[2 * l])
            ...
            y = self.act(y)
            y = self.dropout_layer(y)
            y = y + pad_layer(self.spattention_layers[2 * l + 1](y, cond), self.cond_affine_layers[2 * l + 1])
...
if __name__ == '__main__':
    with open('config.yaml') as f:
        config = yaml.load(f)
    t = torch.rand([1, 80, 124])
    pad_len = 0
    while ceil(t.shape[-1] / 4) % 2 != 0 or ceil(t.shape[-1] / 2) % 2 != 0 or ceil(t.shape[-1]) % 2 != 0:
